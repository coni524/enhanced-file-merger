...
output:
  ...
  max_total_size_kb: 200    # 合計ファイルサイズ制限（デフォルト：200KB）
```

### ファイルサイズ制限について

巨大なファイルが誤って作成されることを防ぐため、デフォルトでは結合後のファイルの合計サイズは200KBに制限されています。この制限は設定ファイルで調整可能です：

```yaml
output:
  max_total_size_kb: 500  # 500KBに変更する例
```

より大きなファイルを処理する必要がある場合は、以下の対応を検討してください：
1. 設定ファイルの `max_total_size_kb` 値を増やす
2. ソースファイルを小さな単位に分割する
3. `--exclude` オプションを使用して大きなファイルを除外する
